...
//		$item->Visible = FALSE;
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'CustomView1_timeta') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_CustomView1_timeta"])) {
			$this->LoadSelectionFromSession('timeta');
		} elseif (@$_SESSION["sel_CustomView1_timeta"] == EWR_INIT_VALUE) { // Select all
			$this->timeta->SelectionList = "";
		}
	}
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->ClearExtFilter == 'CustomView1_timeta')
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_CustomView1_timeta'] = ($this->timeta->SelectionList == "") ? EWR_INIT_VALUE : $this->timeta->SelectionList;
		}
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_CustomView1_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_CustomView1_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_CustomView1_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_CustomView1_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_CustomView1_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_CustomView1_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $parm) {
		$_SESSION['sv_CustomView1_' . $parm] = $sv;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_CustomView1_' . $parm] = $sv1;
		$_SESSION['so_CustomView1_' . $parm] = $so1;
		$_SESSION['sc_CustomView1_' . $parm] = $sc;
		$_SESSION['sv2_CustomView1_' . $parm] = $sv2;
		$_SESSION['so2_CustomView1_' . $parm] = $so2;
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field timeta
		$this->SetDefaultExtFilter($this->timeta, "BETWEEN", NULL, 'AND', "=", NULL);
	...
	function RestoreFilterList() {
		...
		if (array_key_exists("sel_timeta", $filter)) {
			...
			$_SESSION["sel_CustomView1_timeta"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "timeta");
			$this->timeta->SelectionList = "";
			$_SESSION["sel_CustomView1_timeta"] = "";
		}
	}
...
// Form object
var fCustomView1rpt = new ewr_Form("fCustomView1rpt");

// Validate method
fCustomView1rpt.Validate = function() {
...
fCustomView1rpt.Form_CustomValidate = 
...
fCustomView1rpt.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fCustomView1rpt.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<div id="fCustomView1rpt_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<div id="r_1" class="ewRow">
<div id="c_timeta" class="ewCell form-group">
	<label for="sv_timeta" class="ewSearchCaption ewLabel"><?php echo $Page->timeta->FldCaption() ?></label>
	<span class="ewSearchOperator"><?php echo $ReportLanguage->Phrase("BETWEEN"); ?><input type="hidden" name="so_timeta" id="so_timeta" value="BETWEEN"></span>
	<span class="control-group ewSearchField">
<?php ewr_PrependClass($Page->timeta->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" id="sv_timeta" name="sv_timeta" placeholder="<?php echo $Page->timeta->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->timeta->SearchValue) ?>" data-calendar="true" data-format="%d/%m/%Y"<?php echo $Page->timeta->EditAttributes() ?>>
</span>
	<span class="ewSearchCond btw1_timeta"><?php echo $ReportLanguage->Phrase("AND") ?></span>
	<span class="ewSearchField btw1_timeta">
<?php ewr_PrependClass($Page->timeta->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" id="sv2_timeta" name="sv2_timeta" placeholder="<?php echo $Page->timeta->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->timeta->SearchValue2) ?>" data-calendar="true" data-format="%d/%m/%Y"<?php echo $Page->timeta->EditAttributes() ?>>
</span>
</div>
</div>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Count28messages_message29"><div class="CustomView1_Count28messages_message29"><span class="ewTableHeaderCaption"><?php echo $Page->Count28messages_message29->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Count28messages_message29">
<?php if ($Page->SortUrl($Page->Count28messages_message29) == "") { ?>
		<div class="ewTableHeaderBtn CustomView1_Count28messages_message29">
			<span class="ewTableHeaderCaption"><?php echo $Page->Count28messages_message29->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer CustomView1_Count28messages_message29" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Count28messages_message29) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Count28messages_message29->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="timeta"><div class="CustomView1_timeta"><span class="ewTableHeaderCaption"><?php echo $Page->timeta->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="timeta">
<?php if ($Page->SortUrl($Page->timeta) == "") { ?>
		<div class="ewTableHeaderBtn CustomView1_timeta">
			<span class="ewTableHeaderCaption"><?php echo $Page->timeta->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'CustomView1_timeta', true, '<?php echo $Page->timeta->RangeFrom; ?>', '<?php echo $Page->timeta->RangeTo; ?>');" id="x_timeta<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer CustomView1_timeta" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->timeta) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->timeta->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_CustomView1_Count28messages_message29"<?php echo $Page->Count28messages_message29->ViewAttributes() ?>><?php echo $Page->Count28messages_message29->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->timeta->Visible) { ?>
		<td data-field="timeta"<?php echo $Page->timeta->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_CustomView1_timeta"<?php echo $Page->timeta->ViewAttributes() ?>><?php echo $Page->timeta->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<div id="ewBottom" class="ewBottom">
	<!-- Bottom slot -->
<a id="cht_CustomView1_Chart1"></a>
<div class="">
<div id="div_ctl_CustomView1_Chart1" class="ewChart">
<div id="div_CustomView1_Chart1" class="ewChartDiv"></div>
<!-- grid component -->
<div id="div_CustomView1_Chart1_grid" class="ewChartGrid"></div>
</div>
</div>
<?php
...
?>
<?php include_once "CustomView1_Chart1chart.php" ?>
