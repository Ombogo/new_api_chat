...
class cstatus extends cTable {
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("statusview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("statusview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "statusadd.php?" . $this->UrlParm($parm);
		else
			return "statusadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("statusedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("statusadd.php", $this->UrlParm($parm));
	}
   ...
   // Common render codes
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->profile_pix->Exportable) $Doc->ExportCaption($this->profile_pix);
					if ($this->autoreply->Exportable) $Doc->ExportCaption($this->autoreply);
				} else {
					if ($this->status->Exportable) $Doc->ExportCaption($this->status);
					if ($this->profile_pix->Exportable) $Doc->ExportCaption($this->profile_pix);
					if ($this->autoreply->Exportable) $Doc->ExportCaption($this->autoreply);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->profile_pix->Exportable) $Doc->ExportField($this->profile_pix);
						if ($this->autoreply->Exportable) $Doc->ExportField($this->autoreply);
					} else {
						if ($this->status->Exportable) $Doc->ExportField($this->status);
						if ($this->profile_pix->Exportable) $Doc->ExportField($this->profile_pix);
						if ($this->autoreply->Exportable) $Doc->ExportField($this->autoreply);
	...
	function GetUserIDSubquery(&$fld, &$masterfld) {
		global $conn;
		$sWrk = "";
		$sSql = "SELECT " . $masterfld->FldExpression . " FROM `status`";
