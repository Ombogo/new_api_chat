...
class cview1 extends cTable {
	...
	function __construct() {
		...
		$this->Count28messages_message29 = new cField('view1', 'view1', 'x_Count28messages_message29', 'Count(messages.message)', '`Count(messages.message)`', '`Count(messages.message)`', 20, -1, FALSE, '`Count(messages.message)`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// dtime
		$this->dtime = new cField('view1', 'view1', 'x_dtime', 'dtime', '`dtime`', '`dtime`', 200, -1, FALSE, '`dtime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "view1add.php?" . $this->UrlParm($parm);
		else
			return "view1add.php";
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		// Count(messages.message)
		$this->Count28messages_message29->EditAttrs["class"] = "form-control";
		$this->Count28messages_message29->EditCustomAttributes = "";
		$this->Count28messages_message29->EditValue = ew_HtmlEncode($this->Count28messages_message29->CurrentValue);
		$this->Count28messages_message29->PlaceHolder = ew_RemoveHtml($this->Count28messages_message29->FldCaption());
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->Count28messages_message29->Exportable) $Doc->ExportCaption($this->Count28messages_message29);
				} else {
					if ($this->Count28messages_message29->Exportable) $Doc->ExportCaption($this->Count28messages_message29);
				}
				$Doc->EndExportRow();
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->Count28messages_message29->Exportable) $Doc->ExportField($this->Count28messages_message29);
					} else {
						if ($this->Count28messages_message29->Exportable) $Doc->ExportField($this->Count28messages_message29);
					}
					$Doc->EndExportRow();
