...
function ew_AutoLoad($class) {
	...
	if (substr($class, 0, 1) == "c") {
		if ($class == "cmywhatsa_what_db") {
			$file = "mywhatsa_whatdb.php";
		} else {
		$fn = "%cls%info.php";
...
function CurrentProjectID() {
	...
	return "{A5768E41-1B3D-4EFB-93F0-AE1113F97625}";
}
...
class cAdvancedSecurity {
	...
	function ValidateUser(&$usr, &$pwd, $autologin) {
		...
		if (!$ValidateUser) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('pass'), $pwd);
					if ($ValidateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('login_id')); // Load user name
						$this->setSessionUserID($rs->fields('client_id')); // Load User ID
	...
	function GetUserIDByUserName($UserName) {
		...
		if (strval($UserName) <> "") {
			...
			$sSql = $UserTable->GetSQL($sFilter, '');
			if (($RsUser = $conn->Execute($sSql)) && !$RsUser->EOF) {
				$UserID = $RsUser->fields('client_id');
				$RsUser->Close();
				return $UserID;
			}
		}
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $conn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('client_id'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
