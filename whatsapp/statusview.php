...
$status_view = NULL; // Initialize page object first

class cstatus_view extends cstatus {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "statuslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "statuslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "statuslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($status_view)) $status_view = new cstatus_view();

// Page init
$status_view->Page_Init();

// Page main
$status_view->Page_Main();
...
// Page object
var status_view = new ew_Page("status_view");
status_view.PageID = "view"; // Page ID
var EW_PAGE_ID = status_view.PageID; // For backward compatibility

// Form object
var fstatusview = new ew_Form("fstatusview");

// Form_CustomValidate event
fstatusview.Form_CustomValidate = 
...
fstatusview.ValidateRequired = true;
<?php } else { ?>
fstatusview.ValidateRequired = false; 
<?php } ?>
...
<?php
	foreach ($status_view->OtherOptions as &$option)
		$option->Render("body");
?>
...
?>
<form name="fstatusview" id="fstatusview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($status_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $status_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="status">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($status->status->Visible) { // status ?>
	<tr id="r_status">
		<td><span id="elh_status_status"><?php echo $status->status->FldCaption() ?></span></td>
		<td<?php echo $status->status->CellAttributes() ?>>
<span id="el_status_status">
<span<?php echo $status->status->ViewAttributes() ?>>
<?php echo $status->status->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($status->profile_pix->Visible) { // profile_pix ?>
	<tr id="r_profile_pix">
		<td><span id="elh_status_profile_pix"><?php echo $status->profile_pix->FldCaption() ?></span></td>
...
<?php echo $status->profile_pix->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($status->autoreply->Visible) { // autoreply ?>
	<tr id="r_autoreply">
		<td><span id="elh_status_autoreply"><?php echo $status->autoreply->FldCaption() ?></span></td>
		<td<?php echo $status->autoreply->CellAttributes() ?>>
<span id="el_status_autoreply">
<span<?php echo $status->autoreply->ViewAttributes() ?>>
<?php echo $status->autoreply->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
