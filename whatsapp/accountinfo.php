...
class caccount extends cTable {
	...
	function __construct() {
		...
		// client_id
		$this->client_id = new cField('account', 'account', 'x_client_id', 'client_id', '`client_id`', '`client_id`', 3, -1, FALSE, '`client_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->client_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['client_id'] = &$this->client_id;

		// login_id
		$this->login_id = new cField('account', 'account', 'x_login_id', 'login_id', '`login_id`', '`login_id`', 200, -1, FALSE, '`login_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->login_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['login_id'] = &$this->login_id;

		// name
		$this->name = new cField('account', 'account', 'x_name', 'name', '`name`', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['name'] = &$this->name;

		// pass
		$this->pass = new cField('account', 'account', 'x_pass', 'pass', '`pass`', '`pass`', 200, -1, FALSE, '`pass`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('client_id', $rs))
				ew_AddFilter($where, ew_QuotedName('client_id') . '=' . ew_QuotedValue($rs['client_id'], $this->client_id->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("accountview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("accountview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "accountadd.php?" . $this->UrlParm($parm);
		else
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["client_id"]; // client_id
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->client_id->CurrentValue = $key;
   ...
   // Common render codes
		...
		// client_id
		$this->client_id->LinkCustomAttributes = "";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->client_id->Exportable) $Doc->ExportCaption($this->client_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->client_id->Exportable) $Doc->ExportField($this->client_id);
	...
	function UserIDFilter($userid) {
		$sUserIDFilter = '`client_id` = ' . ew_QuotedValue($userid, EW_DATATYPE_NUMBER);
