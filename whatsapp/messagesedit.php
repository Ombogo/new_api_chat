...
class cmessages_edit extends cmessages {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("messageslist.php"); // Invalid key, return to list
...
return true;
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->pnumber->FldIsDetailKey) {
			$this->pnumber->setFormValue($objForm->GetValue("x_pnumber"));
		}
...
$SMS1 = "http://furahiamobile.com/whatsapp/auto.php?targer=".$objForm->GetValue("x_pnumber")."&SMS=".$objForm->GetValue("x_message");

curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);

curl_setopt($ch, CURLOPT_URL, $SMS1);
...
curl_exec($ch);
	...
	function RestoreFormValues() {
		...
		$this->pnumber->CurrentValue = $this->pnumber->FormValue;
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, "messageslist.php", "", $this->TableVar, TRUE);
		$PageId = "edit";
...
// Page object
var messages_edit = new ew_Page("messages_edit");
messages_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = messages_edit.PageID; // For backward compatibility

// Form object
var fmessagesedit = new ew_Form("fmessagesedit");
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $messages->message->FldCaption(), $messages->message->ReqErrMsg)) ?>");
...
fmessagesedit.ValidateRequired = true;

<?php } else { ?>
fmessagesedit.ValidateRequired = false; 
<?php } ?>
...
?>
<form name="fmessagesedit" id="fmessagesedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<?php if ($messages->pnumber->Visible) { // pnumber ?>
	<div id="r_pnumber" class="form-group">
		<label id="elh_messages_pnumber" for="x_pnumber" class="col-sm-2 control-label ewLabel"><?php echo $messages->pnumber->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $messages->pnumber->CellAttributes() ?>>
<span id="el_messages_pnumber">
<span<?php echo $messages->pnumber->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $messages->pnumber->EditValue ?></p></span>
</span>
<input type="hidden" data-field="x_pnumber" name="x_pnumber" id="x_pnumber" value="<?php echo ew_HtmlEncode($messages->pnumber->CurrentValue) ?>">
<?php echo $messages->pnumber->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($messages->message->Visible) { // message ?>
	<div id="r_message" class="form-group">
		<label id="elh_messages_message" for="x_message" class="col-sm-2 control-label ewLabel"><?php echo $messages->message->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $messages->message->CellAttributes() ?>>
<span id="el_messages_message">
<textarea data-field="x_message" name="x_message" id="x_message" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($messages->message->PlaceHolder) ?>"<?php echo $messages->message->EditAttributes() ?>><?php echo $messages->message->EditValue ?></textarea>
</span>
<?php echo $messages->message->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($messages->id->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
	<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo "Reply" ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
