...
class cstatus_delete extends cstatus {
	...
	function Page_Main() {
		...
		if ($this->Recordset = ew_LoadRecordset($sql)) {
			...
			if (!$res) $this->Page_Terminate("statuslist.php"); // Return to list
		}
...
// Page object
var status_delete = new ew_Page("status_delete");
status_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = status_delete.PageID; // For backward compatibility

// Form object
var fstatusdelete = new ew_Form("fstatusdelete");
...
if ($status_delete->Recordset = $status_delete->LoadRecordset())
	$status_deleteTotalRecs = $status_delete->Recordset->RecordCount(); // Get record count
if ($status_deleteTotalRecs <= 0) { // No record found, exit
	if ($status_delete->Recordset)
		$status_delete->Recordset->Close();
	$status_delete->Page_Terminate("statuslist.php"); // Return to list
}
?>
<div class="ewToolbar">
...
<?php } ?>
<?php if ($status->account_id->Visible) { // account_id ?>
		<th><span id="elh_status_account_id" class="status_account_id"><?php echo $status->account_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($status->profile_pix->Visible) { // profile_pix ?>
		<th><span id="elh_status_profile_pix" class="status_profile_pix"><?php echo $status->profile_pix->FldCaption() ?></span></th>
<?php } ?>
<?php if ($status->autoreply->Visible) { // autoreply ?>
		<th><span id="elh_status_autoreply" class="status_autoreply"><?php echo $status->autoreply->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
...
<?php } ?>
<?php if ($status->account_id->Visible) { // account_id ?>
		<td<?php echo $status->account_id->CellAttributes() ?>>
<span id="el<?php echo $status_delete->RowCnt ?>_status_account_id" class="status_account_id">
<span<?php echo $status->account_id->ViewAttributes() ?>>
<?php echo $status->account_id->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($status->profile_pix->Visible) { // profile_pix ?>
		<td<?php echo $status->profile_pix->CellAttributes() ?>>
<span id="el<?php echo $status_delete->RowCnt ?>_status_profile_pix" class="status_profile_pix">
...
<?php } ?>
<?php if ($status->autoreply->Visible) { // autoreply ?>
		<td<?php echo $status->autoreply->CellAttributes() ?>>
<span id="el<?php echo $status_delete->RowCnt ?>_status_autoreply" class="status_autoreply">
